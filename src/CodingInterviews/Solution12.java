...
public class Solution12 {
    public boolean exist(char[][] board, String word) {
        ...
        for(int i=0;i<board.length;i++){
            for(int j=0;j<board[0].length;j++){
                if(temp[0]==board[i][j]){
                    if(exist(board,visit,temp,i,j,0))   return true;
                }
            }
        }
        return false;
    ...
    private boolean exist(char[][] board,boolean[][] visit,char[] word,int i,int j,int index){
        if(index==word.length){
            return true;
        }
        if(i<0 || j<0 || i==board.length || j==board[0].length || visit[i][j] || board[i][j]!=word[index]){
            return false;
        }
        visit[i][j]=true;
        boolean res=exist(board,visit,word,i+1,j,index+1)
                || exist(board,visit,word,i-1,j,index+1)
                || exist(board,visit,word,i,j+1,index+1)
                || exist(board,visit,word,i,j-1,index+1);
        visit[i][j]=false;
        return res;
    }
